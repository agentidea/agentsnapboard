...
namespace AgentStoryComponents
{
    public class Page
    {
        ...
        private void LoadUserViewers(int pageID)
        {
            OleDbHelper dbHelper = ute.getDBcmd(this._connectionString);

            string sql = "";

            sql += "Select user_id from PageUserViewers  WHERE pageID = " + pageID;

            dbHelper.cmd.CommandText = sql;

            dbHelper.reader = dbHelper.cmd.ExecuteReader();

            this.PageViewerUsers.Clear();

            while (dbHelper.reader.Read())
            {
                int userID = (int)dbHelper.reader["user_id"];
                User u = new User(config.conn, userID);
                this.PageViewerUsers.Add(u);
            }
        ...
        private void LoadUserEditors(int pageID)
        {
            OleDbHelper dbHelper = ute.getDBcmd(this._connectionString);

            string sql = "";

            sql += "Select user_id from PageUserEditors  WHERE pageID = " + pageID;

            dbHelper.cmd.CommandText = sql;

            dbHelper.reader = dbHelper.cmd.ExecuteReader();

            this.PageEditorUsers.Clear();

            while (dbHelper.reader.Read())
            {
                int userID = (int)dbHelper.reader["user_id"];
                User u = new User(config.conn, userID);
                this.PageEditorUsers.Add(u);
            }
        ...
        private void LoadGroupViewers(int pageID)
        {
            OleDbHelper dbHelper = ute.getDBcmd(this._connectionString);

            string sql = "";

            sql += "Select group_id from PageGroupViewers  WHERE pageID = " + pageID;

            dbHelper.cmd.CommandText = sql;

            dbHelper.reader = dbHelper.cmd.ExecuteReader();

            this.PageViewerGroups.Clear();

            while (dbHelper.reader.Read())
            {
                int groupId = (int)dbHelper.reader["group_id"];
                Group g = new Group(config.conn, groupId);
                this.PageViewerGroups.Add(g);
            }
        ...
        private void LoadGroupEditors(int pageID)
        {
            OleDbHelper dbHelper = ute.getDBcmd(this._connectionString);

            string sql = "";

            sql += "Select group_id from PageGroupEditors  WHERE pageID = " + pageID;

            dbHelper.cmd.CommandText = sql;

            dbHelper.reader = dbHelper.cmd.ExecuteReader();

            this.PageEditorGroups.Clear();

            while (dbHelper.reader.Read())
            {
                int groupId = (int)dbHelper.reader["group_id"];
                Group g = new Group(config.conn, groupId);
                this.PageEditorGroups.Add(g);
            }
        ...
        public void AddUserViewer(User u)
        {
            ...
            sql += "INSERT INTO PageUserViewers ( pageID,user_id,dateAdded ) VALUES ( ";
        ...
        public void RemoveUserViewer(User u)
        {
            ...
            sql += "DELETE FROM PageUserViewers WHERE ";
            sql += " pageID = " + this.ID;
        ...
        public void AddUserEditor(User u)
        {
            ...
            sql += "INSERT INTO PageUserEditors ( pageID,user_id,dateAdded ) VALUES ( ";
        ...
        public void RemoveUserEditor(User u)
        {
            ...
            sql += "DELETE FROM PageUserEditors WHERE ";
            sql += " pageID = " + this.ID;
            sql += " AND user_id = " + u.ID;
        ...
        public void AddGroupViewer(Group g)
        {
            ...
            sql += "INSERT INTO PageGroupViewers ( pageID,group_id,dateAdded ) VALUES ( ";
        ...
        public void RemoveGroupViewer(Group g)
        {
            ...
            sql += "DELETE FROM PageGroupViewers WHERE ";
            sql += " pageID = " + this.ID;
        ...
        public void AddGroupEditor(Group g)
        {
            ...
            sql += "INSERT INTO PageGroupEditors ( pageID,group_id,dateAdded ) VALUES ( ";
        ...
        public void RemoveGroupEditor(Group g)
        {
            ...
            sql += "DELETE FROM PageGroupEditors WHERE ";
            sql += " pageID = " + this.ID;
            sql += " AND group_id = " + g.ID;
        ...
        public selWidget getUserEditors()
        {
            ...
            foreach (User u in this.PageEditorUsers)
            {
                tmpSB.Append(u.ID);
                tmpSB.Append("|");
            ...
            if (this.PageEditorUsers.Count > 0)
            {
        ...
        public selWidget getUserViewers()
        {
            ...
            foreach (User u in this.PageViewerUsers)
            {
                tmpSB.Append(u.ID);
                tmpSB.Append("|");
            ...
            if (this.PageViewerUsers.Count > 0)
            {
                tmpSB.Remove(tmpSB.Length - 1, 1);
                tmpSB2.Remove(tmpSB2.Length - 1, 1);
            }
        ...
        public selWidget getGroupEditors()
        {
            ...
            foreach (Group g in this.PageEditorGroups)
            {
                tmpSB.Append(g.ID);
                tmpSB.Append("|");
            ...
            if (this.PageEditorGroups.Count > 0)
            {
                tmpSB.Remove(tmpSB.Length - 1, 1);
                tmpSB2.Remove(tmpSB2.Length - 1, 1);
            }
        ...
        public selWidget getGroupViewers()
        {
            ...
            foreach (Group g in this.PageViewerGroups)
            {
                tmpSB.Append(g.ID);
                tmpSB.Append("|");
            ...
            if (this.PageViewerGroups.Count > 0)
            {
                tmpSB.Remove(tmpSB.Length - 1, 1);
                tmpSB2.Remove(tmpSB2.Length - 1, 1);
            }
        ...
        public void decoratePageACL2(User _who)
        {
            ...
            {
                foreach (Group storyGroup in this.PageViewerGroups)
                {
            ...
            if (this.CanView == false)
            {
                ...
                foreach (User storyUser in this.PageViewerUsers)
                {
            ...
            foreach (Group userGroup in theirGroups)
            {
                foreach (Group storyGroup in this.PageEditorGroups)
                {
                    if (userGroup.ID == storyGroup.ID)
                    {
                        this._canEdit = true;
            ...
            if (this._canEdit == false)
            {
                ...
                foreach (User storyUser in this.PageEditorUsers)
                {
                    if (userID == storyUser.ID)
                    {
                        this._canEdit = true;
