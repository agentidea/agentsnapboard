...
function newStoryEditor(storyController)
{
...
function StoryEditor()
{
   ...
   // this.getDebugPanel = getDebugPanel;
    ...
    var _palette = null;
    this.MainPalette = _palette;
    //this.palette_callback = palette_callback;
    
    this.getPalette = getPalette;
    this.getEditorMainGrid = getEditorMainGrid;
...
function initStoryEditor(storyController)
{
    ...
    else
    {
        ...
        //$TODO user bookmarks!!!!
        storyController.setCurrentPageCursor( 0 );
        this.loadCurrPage();
    }
    
    
    var valuesOuter = new Array();
    valuesOuter[0] = this.getEditorMainGrid();
    
    
    valuesOuter[1] = this.getPalette();

    var oGrid3 = newGrid2("storyEditorOuterMainGrid",1,2,valuesOuter,1);
    oGrid3.init( oGrid3 );
    this.container = oGrid3.gridTable;
...
function getPalette()
{
    ...
    divPalette.className="clsPalette";
    
    var p = new palette("YouTube",null);
    p.init();
    
    divPalette.appendChild( p.container );
    values[0] = divPalette;
    ...
    var oGrid2 = newGrid2("palette",1,1,values,5);
    oGrid2.init( oGrid2 );
    return oGrid2.gridTable;
}

function getEditorMainGrid()
{
    var values = new Array();
...
function showJason()
{
    var json = oStoryEditor.StoryController.CurrentStory.toJSONString();
    oStoryEditor.log( json );
}
...
{
    var cols = this.StoryController.GetPage(this.StoryController.getCurrentPageCursor()).gridCols;
    var rows = this.StoryController.GetPage(this.StoryController.getCurrentPageCursor()).gridRows;
...
{
   oStoryEditor.loadCurrPage();
   oStoryEditor.loadPageCursor();
   oStoryEditor.loadPageName();
...
function pageNav_change()
{
    ...
    var prevCursor = oStoryEditor.StoryController.getCurrentPageCursor();
    oStoryEditor.StoryController.setCurrentPageCursor(nPageCursor);
    oStoryEditor.displayPageChange();
...
function page_next()
{
    ...
    //get curr page
    if( oStoryEditor.StoryController.getCurrentPageCursor() == oStoryEditor.StoryController.CurrentStory.PageCount - 1)
    {   //reset
        oStoryEditor.StoryController.setCurrentPageCursor(  0 );
    }
    else
    {
        oStoryEditor.StoryController.IncrementPageCursor();
    }
...
{
    //get curr page
    if( oStoryEditor.StoryController.getCurrentPageCursor() == 0)
    {   //reset
        oStoryEditor.StoryController.setCurrentPageCursor(oStoryEditor.StoryController.CurrentStory.PageCount - 1);
    }
    else
    {
        oStoryEditor.StoryController.DecrementPageCursor();
    }
...
function page_delete()
{
    ...
    var currPageCursor = oStoryEditor.StoryController.getCurrentPageCursor();

    var p = oStoryEditor.StoryController.GetPage( currPageCursor );
    if(p != null)
    {
    ...
    if(res)
    {
       ...
       addParam( macroCreateNewPage,"StoryID",oStoryEditor.StoryController.CurrentStory.ID );
...
function newPage_add()
{
    ...
    if(pageName != null)
    {
      ...
      pageName = TheUte().encode64( TheUte().filterText( pageName ) );
       ...
       addParam( macroCreateNewPage,"gridLayers",-1);
       addParam( macroCreateNewPage,"StoryID",oStoryEditor.StoryController.CurrentStory.ID );
       addParam( macroCreateNewPage,"StoryOpenedBy",oStoryEditor.StoryController.CurrentStory.StoryOpenedBy );
...
function loadPageName()
{
    ...
    var currPageCursor = this.StoryController.getCurrentPageCursor();
...
function save_page_name_clicked()
{
    ...
    oStoryEditor.lastGoodPageName = txtPageName.value;
...
{
    var txtPageName = TheUte().findElement("txtPageName","input");
    txtPageName.value =  oStoryEditor.lastGoodPageName;
...
function save_page_name()
{
    ...
    var currPage = oStoryEditor.StoryController.GetPage(oStoryEditor.StoryController.getCurrentPageCursor());
    oStoryEditor.updatePageInfo(currPage.ID,currPage.gridCols,currPage.gridRows,oStoryEditor.StoryController.CurrentStory.ID,pageName );
...
function loadPageCursor()
{
    ...
    var txtCursor = document.createTextNode( this.StoryController.getCurrentPageCursor() + 1 );
...
function refreshPageNavList()
{
    ...
    var pageNavList = oStoryEditor.getPageNavList();
    if(pageNavList != null)
...
function Grid_ContractExpand()
{
    ...
    var thePage = oStoryEditor.StoryController.GetPage(oStoryEditor.StoryController.getCurrentPageCursor());
	...
	//load new grid
	var newGrid = oStoryEditor.getPageGrid();
	if( newGrid != null)
	{
	  ...
	  // alert("update this page " + thePage.Name + " with new grid dimensions " + rows + " x " + cols);
       ...
       oStoryEditor.updatePageInfo(currPage.ID,cols,rows,oStoryEditor.StoryController.CurrentStory.ID,currPage.Name);
...
function decorateGrid()
{
    ...
    var currPage = this.StoryController.GetPage(oStoryEditor.StoryController.getCurrentPageCursor());
    ...
    for(var i = 0; i<numberOfCells;i++)
    {
        ...
        var pe = this.StoryController.FindPageElementByCoord(oStoryEditor.StoryController.CurrentStory,currPage,colCursor,rowCursor);
...
function txtCell_Content_Clicked()
{
    ...
    if(this.value.trim().length == 0)
    {
        ...
        oStoryEditor.ThePageElementEditor.changeMediaOptions(1);
...
function PageElement_Update()
{
    ...
    var currPageElem = oStoryEditor.ThePageElementEditor.CurrentPageElement;
    var posX = oStoryEditor.ThePageElementEditor.CurrGridX ;
    var posY = oStoryEditor.ThePageElementEditor.CurrGridY ;

    var sContentVal = oStoryEditor.ThePageElementEditor.msgBox.value;
    ...
    if( sContentValFiltered != sContentVal)
    {
        ...
        oStoryEditor.ThePageElementEditor.msgBox.value = sContentValFiltered;
    }
    ...
    if( oStoryEditor.ThePageElementEditor.TypeID == -1 )
    {
       //set to be text
        oStoryEditor.ThePageElementEditor.changeMediaOptions(1);
    }
    ...
    if( currPageElem == null )
    {
       ...
       addParam( macroCreateNewPageElementAndMap,"CurrentPageCursor",oStoryEditor.StoryController.getCurrentPageCursor());
       ...
       addParam( macroCreateNewPageElementAndMap,"GridZ",-1 + "");
       addParam( macroCreateNewPageElementAndMap,"Value",sContentVal64);
       addParam( macroCreateNewPageElementAndMap,"tags",oStoryEditor.ThePageElementEditor.tagBox.value);
       addParam( macroCreateNewPageElementAndMap,"TypeID",oStoryEditor.ThePageElementEditor.TypeID );
       addParam( macroCreateNewPageElementAndMap,"StoryID",oStoryEditor.StoryController.CurrentStory.ID );
       addParam( macroCreateNewPageElementAndMap,"StoryOpenedBy",oStoryEditor.StoryController.CurrentStory.StoryOpenedBy );
    ...
    else
    {
       ...
       addParam( macroUpdatePageElementAndMap,"GridZ",-1 + "");
       addParam( macroUpdatePageElementAndMap,"PageElementMapGUID", "");
       addParam( macroUpdatePageElementAndMap,"StoryID",oStoryEditor.StoryController.CurrentStory.ID);
       addParam( macroUpdatePageElementAndMap,"Value",sContentVal64);
       addParam( macroUpdatePageElementAndMap,"tags",oStoryEditor.ThePageElementEditor.tagBox.value);
       addParam( macroUpdatePageElementAndMap,"TypeID",oStoryEditor.ThePageElementEditor.TypeID );
       addParam( macroUpdatePageElementAndMap,"StoryOpenedBy",oStoryEditor.StoryController.CurrentStory.StoryOpenedBy );
   ...
   // alert(oStoryEditor.StoryController.CurrentStory.toJSONString() );
}
function PageElement_Preview()
{
    oStoryEditor.ThePageElementEditor.divPreview.innerHTML = "&nbsp;";
    oStoryEditor.ThePageElementEditor.divPreview.innerHTML = oStoryEditor.ThePageElementEditor.msgBox.value;
}

function ContentEditor_Clear()
{
    oStoryEditor.ThePageElementEditor.msgBox.value = "";
    oStoryEditor.ThePageElementEditor.divPreview.innerHTML = "&nbsp;";
}
...
function mediaTypeChoice_change()
{
   ...
   var indexMediaOptions = this.value;
    ...
    oStoryEditor.ThePageElementEditor.changeMediaOptions( indexMediaOptions );
    this.selectedIndex = 0;
}
...
function changeMediaOptions(indx)
{
    ...
    oStoryEditor.ThePageElementEditor.TypeID = indexMediaOptions;
    oStoryEditor.ThePageElementEditor.Type = oStoryEditor.ThePageElementEditor.mediaOptions[ indexMediaOptions ];
    oStoryEditor.ThePageElementEditor.changeDisplayOfType( oStoryEditor.ThePageElementEditor.Type );
    
    var cleanSnippetText = oStoryEditor.getSnippet(indexMediaOptions);
    var prevSnippetText = null;
    
    oStoryEditor.getSnippet(oStoryEditor.ThePageElementEditor.prevSelectedTypeIndex);//this.
    var msgBoxText = oStoryEditor.ThePageElementEditor.msgBox.value.trim();
    
    if( msgBoxText.length == 0 )
    {
        oStoryEditor.ThePageElementEditor.msgBox.value = cleanSnippetText;
    }
    else
    {
        //there was stuff in the box, has it changed?
        if(oStoryEditor.ThePageElementEditor.prevSelectedTypeIndex > 0)
        {
            prevSnippetText = oStoryEditor.getSnippet(oStoryEditor.ThePageElementEditor.prevSelectedTypeIndex);
            if(prevSnippetText == oStoryEditor.ThePageElementEditor.msgBox.value.trim())
            {
    ...
    oStoryEditor.ThePageElementEditor.prevSelectedTypeIndex = indexMediaOptions;
}
...
function pageElement_Selected()
{
   ...
   var posY = cartesianBits[1] * 1;
    ...
    oStoryEditor.ThePageElementEditor.CurrGridX = posX;
    oStoryEditor.ThePageElementEditor.CurrGridY = posY;
    
    //alert(oStoryEditor.ThePageElementEditor.msgBox.value);
    if( oStoryEditor.ThePageElementEditor.msgBox.value.trim().length == 0 || oStoryEditor.ThePageElementEditor.bCommited == true )
    {
        oStoryEditor.ThePageElementEditor.msgBox.value = "";
        oStoryEditor.ThePageElementEditor.tagBox.value = "";
        oStoryEditor.ThePageElementEditor.divPreview.innerHTML = "<DIV></DIV>";
        oStoryEditor.ThePageElementEditor.bCommited = false;
    }
   ...
   var page = null;
   page = oStoryEditor.StoryController.GetPage(oStoryEditor.StoryController.getCurrentPageCursor());
   var pe = oStoryEditor.StoryController.FindPageElementByCoord(oStoryEditor.StoryController.CurrentStory,page, posX,posY);

   var sType = null;
   
   if(pe != null)
   {
       oStoryEditor.ThePageElementEditor.CurrentPageElement = pe;
       oStoryEditor.ThePageElementEditor.msgBox.value = TheUte().decode64(pe.Value);
       oStoryEditor.ThePageElementEditor.tagBox.value = pe.Tags;
       oStoryEditor.ThePageElementEditor.divPreview.innerHTML = TheUte().decode64(pe.Value);
       sType = pe.Type;
       oStoryEditor.ThePageElementEditor.Type = pe.Type;
       oStoryEditor.ThePageElementEditor.TypeID = pe.TypeID;
       
   }
   else
   {
        //bug fix for the null page element, should always be regardless of msgBox.len
        //no page element set to null NOR no content
        oStoryEditor.ThePageElementEditor.CurrentPageElement = null;
            
        if(oStoryEditor.ThePageElementEditor.msgBox.value.trim().length == 0)
        {
           
            oStoryEditor.ThePageElementEditor.Type = "nil";
            oStoryEditor.ThePageElementEditor.TypeID = -1;
            
            sType = "nil";
        }
        else
        {
            sType = oStoryEditor.ThePageElementEditor.Type;
        }
   }

    oStoryEditor.ThePageElementEditor.changeDisplayOfType(sType);
    oStoryEditor.ThePageElementEditor.changeDisplayOfCoord(oStoryEditor.ThePageElementEditor.CurrGridX, oStoryEditor.ThePageElementEditor.CurrGridY);
...
function changeDisplayOfType(msg)
{
   ...
   TheUte().removeChildren( oStoryEditor.ThePageElementEditor.divPageElementType );
   oStoryEditor.ThePageElementEditor.divPageElementType.appendChild( txtDiv );
}
...
function changeDisplayOfCoord(x,y)
{
   ...
   TheUte().removeChildren( oStoryEditor.ThePageElementEditor.divPageElementCoord  );
   oStoryEditor.ThePageElementEditor.divPageElementCoord.appendChild( txtDiv );
}
function pageElement_Blur()
{
  //  oStoryEditor.ThePageElementEditor.msgBox.value = "";
  //  alert( oStoryEditor.ThePageElementEditor.divPageElementType.innerHTML );
  //  TheUte().removeChildren( oStoryEditor.ThePageElementEditor.divPageElementType );
