...
namespace AgentStoryComponents.extAPI.commands
{
    public class extraSetColVal2 : ICommand
    {
        public MacroEnvelope execute(Macro macro)
        {
            ...
            string tblName = MacroUtils.getParameterString("tblName", macro);
            ...
            //    throw new Exception (string.Format("no table called {0} exists",tblName));

            string sql = string.Format("UPDATE {0} SET {1} = {2} WHERE tx_id = '{3}'",
                                       tblName, colName, colValue, tx_id);
            ...
            Macro proc = new Macro("RefreshTable", 2);              //callback, can be generic?
                                                                    // update model ...
            proc.addParameter("table", tblName);
